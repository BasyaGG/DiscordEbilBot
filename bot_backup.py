...
• Сокращения: `спб`, `питер`, `екб`, `мск`

**💰 Криптовалюты:**
• `!крипта` - основные криптовалюты
• `!крипта <символ>` - конкретная криптовалюта
• `!крипта btc eth` - несколько криптовалют
• Поддержка: BTC, ETH, USDT, BNB, XRP, ADA, DOGE и др.
...
• `!twitch сообщение <канал> <текст>` - настроить уведомление
...
• `!канал сброс` - разрешить работу во всех каналах
...
async def leave_voice(ctx):
    ...
    try:
        ...
        await ctx.reply(f"❌ Ошибка отключения: {str(e)}")

# Удалена дублирующаяся команда радио
...
async def play_simple(ctx, *, query=None):
    """Простая команда воспроизведения (пока без yt-dlp)"""
    if query is None:
        await ctx.reply("❌ Укажите что воспроизвести! Пример: `!play название песни`")
        return
    
    await ctx.reply(f"🎵 Поиск музыки временно недоступен. Попробуйте `!радио` для радио Bluford!")
    # TODO: Добавить yt-dlp функционал позже

# ===== ДОПОЛНИТЕЛЬНЫЕ КОМАНДЫ ДЛЯ ДИАГНОСТИКИ =====
@bot.command(name='voice_info', aliases=['инфо_голос'])
async def voice_info(ctx):
    """Информация о голосовом подключении"""
    if ctx.voice_client is None:
        await ctx.reply("❌ Не подключен к голосовому каналу")
        return
    
    embed = discord.Embed(title="🔊 Информация о голосовом подключении", color=0x00ff00)
    
    embed.add_field(name="Канал", value=ctx.voice_client.channel.name, inline=True)
    embed.add_field(name="Подключен", value="✅ Да", inline=True)
    embed.add_field(name="Играет", value="✅ Да" if ctx.voice_client.is_playing() else "❌ Нет", inline=True)
    embed.add_field(name="На паузе", value="✅ Да" if ctx.voice_client.is_paused() else "❌ Нет", inline=True)
    embed.add_field(name="Задержка", value=f"{ctx.voice_client.latency:.2f}ms", inline=True)
    
    await ctx.reply(embed=embed)

# Удалены дублирующиеся команды radio_test и simple_radio

@bot.command(name='test_stream')
async def test_stream(ctx):
    """Тестовое радио для проверки"""
    if ctx.voice_client is None:
        if ctx.author.voice is None:
            await ctx.reply("❌ Подключитесь к голосовому каналу!")
            return
        await ctx.author.voice.channel.connect()
        await ctx.send("✅ Подключился к каналу")
    
    if ctx.voice_client.is_playing():
        ctx.voice_client.stop()
    
    try:
        # Тестовый поток (обычно работает)
        test_url = "http://stream.radiojar.com/4wqre23fytzuv"
        source = discord.FFmpegPCMAudio(test_url)
        ctx.voice_client.play(source)
        await ctx.send("🎵 Тестовое радио включено")
...
# Дублирующиеся команды join, leave, play удалены
...
async def play_radio(ctx):
    ...
    try:
        ...
        # Настройки FFmpeg для радио потока
        radio_ffmpeg_options = {
            'before_options': (
                '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 '
                '-probesize 10M -analyzeduration 0 -loglevel panic'
            ),
            'options': '-vn -filter:a "volume=0.5"'
        }
        
        # Создаем аудио источник для радио
        source = discord.FFmpegPCMAudio(radio_url, **radio_ffmpeg_options)
        
        # Воспроизводим радио с обработчиком ошибок
        ...
        if ctx.voice_client.is_playing():
            ...
            embed.add_field(name="Статус", value="🔊 Воспроизводится", inline=False)
            embed.set_footer(text=f"Запросил: {ctx.author.display_name}")
            await ctx.send(embed=embed)
        else:
            await ctx.send("⚠️ Радио подключено, но возможны проблемы с потоком")
        
    except Exception as e:
        await ctx.reply(f"❌ Ошибка при подключении к радио: {str(e)}")
        print(f"Ошибка радио: {e}")

# Дублирующиеся команды stop, pause, resume, volume, voice_info, radio_test удалены
        await ctx.send("🔍 Тестирую радио поток...")
        
        # Проверяем доступность потока
        async with aiohttp.ClientSession() as session:
            async with session.get(radio_url, timeout=10) as response:
                if response.status == 200:
                    await ctx.send(f"✅ Радио поток доступен (HTTP {response.status})")
                    content_type = response.headers.get('content-type', 'unknown')
                    await ctx.send(f"📋 Тип контента: {content_type}")
                else:
                    await ctx.send(f"❌ Радио поток недоступен (HTTP {response.status})")
                    return
        ...
        def after_test(error):
            if error:
                print(f'Тест радио - ошибка: {error}')
            else:
                print('Тест радио - успешно')
        
        ctx.voice_client.play(source, after=after_test)
        
        await asyncio.sleep(3)
        
        if ctx.voice_client.is_playing():
            await ctx.send("✅ Радио тест успешен - звук должен воспроизводиться!")
        else:
            await ctx.send("❌ Радио тест неудачен - проверьте настройки Discord")
            
    except Exception as e:
        await ctx.reply(f"❌ Ошибка теста: {str(e)}")

@bot.command(name='simple_radio', aliases=['простое_радио'])
async def simple_radio(ctx):
    """Упрощенная версия радио без дополнительных настроек"""
    if ctx.voice_client is None:
        if ctx.author.voice is None:
            await ctx.reply("❌ Подключитесь к голосовому каналу!")
            return
    ...
    try:
        ...
        await ctx.send("📻 Простое радио включено (без дополнительных настроек)")
        
    except Exception as e:
        await ctx.reply(f"❌ Ошибка: {str(e)}")

@bot.command(name='radio_alt', aliases=['радио_альт'])
async def play_radio_alt(ctx):
    """Включить альтернативное радио для тестирования"""
    if ctx.voice_client is None:
        if ctx.author.voice is None:
            await ctx.reply("❌ Подключитесь к голосовому каналу!")
            return
    ...
    try:
        # Альтернативное радио для тестирования
        radio_url = "http://stream.radiojar.com/4wqre23fytzuv"  # Lofi Hip Hop радио
        await ctx.send("📻 Подключаюсь к альтернативному радио...")
        
        # Простые настройки FFmpeg
        ffmpeg_options = {
            'before_options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5',
        ...
        def after_playing(error):
            if error:
                print(f'Ошибка альтернативного радио: {error}')
            else:
                print('Альтернативное радио остановлено')
        ...
        if ctx.voice_client.is_playing():
            embed = discord.Embed(
                title="📻 Альтернативное радио включено",
                description="**Lofi Hip Hop Radio**\nТестовый поток",
                color=0x00ff00
            )
            embed.add_field(name="Ссылка", value=radio_url, inline=False)
            embed.add_field(name="Статус", value="🔊 Воспроизводится", inline=False)
            embed.set_footer(text=f"Запросил: {ctx.author.display_name}")
            await ctx.send(embed=embed)
        else:
            await ctx.send("⚠️ Альтернативное радио подключено, но возможны проблемы с потоком")
        
    except Exception as e:
        await ctx.reply(f"❌ Ошибка альтернативного радио: {str(e)}")
        print(f"Ошибка альтернативного радио: {e}")

@bot.command(name='radio_debug', aliases=['радио_дебуг'])
async def radio_debug(ctx):
    """Детальная диагностика радио"""
    if ctx.voice_client is None:
        await ctx.reply("❌ Сначала подключитесь командой !join")
        return
    
    try:
        radio_url = "http://bluford.torontocast.com:8085/stream/"
        await ctx.send("🔍 Запускаю детальную диагностику радио...")
        
        # Проверяем доступность потока
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(radio_url, timeout=15) as response:
                    await ctx.send(f"📡 HTTP статус: {response.status}")
                    await ctx.send(f"📋 Content-Type: {response.headers.get('content-type', 'неизвестно')}")
                    await ctx.send(f"📊 Content-Length: {response.headers.get('content-length', 'неизвестно')}")
                    
                    if response.status != 200:
                        await ctx.send(f"❌ Поток недоступен (HTTP {response.status})")
                        return
            except Exception as e:
                await ctx.send(f"❌ Ошибка подключения к потоку: {str(e)}")
                return
        
        # Тестируем разные настройки FFmpeg
        await ctx.send("🔧 Тестирую настройки FFmpeg...")
        
        # Вариант 1: Минимальные настройки
        try:
            source1 = discord.FFmpegPCMAudio(radio_url)
            ctx.voice_client.play(source1)
            await asyncio.sleep(3)
            
            if ctx.voice_client.is_playing():
                await ctx.send("✅ Вариант 1 (минимальные настройки): РАБОТАЕТ")
                return
            else:
                await ctx.send("❌ Вариант 1 (минимальные настройки): НЕ РАБОТАЕТ")
                ctx.voice_client.stop()
        except Exception as e:
            await ctx.send(f"❌ Вариант 1 ошибка: {str(e)}")
        
        # Вариант 2: С буферизацией
        try:
            ffmpeg_options2 = {
                'before_options': '-reconnect 1 -reconnect_streamed 1',
                'options': '-vn -bufsize 2048k'
            }
            source2 = discord.FFmpegPCMAudio(radio_url, **ffmpeg_options2)
            ctx.voice_client.play(source2)
            await asyncio.sleep(3)
            
            if ctx.voice_client.is_playing():
                await ctx.send("✅ Вариант 2 (с буферизацией): РАБОТАЕТ")
                return
            else:
                await ctx.send("❌ Вариант 2 (с буферизацией): НЕ РАБОТАЕТ")
                ctx.voice_client.stop()
        except Exception as e:
            await ctx.send(f"❌ Вариант 2 ошибка: {str(e)}")
        
        # Вариант 3: Opus кодек
        try:
            source3 = discord.FFmpegOpusAudio(radio_url)
            ctx.voice_client.play(source3)
            await asyncio.sleep(3)
            
            if ctx.voice_client.is_playing():
                await ctx.send("✅ Вариант 3 (Opus кодек): РАБОТАЕТ")
                return
            else:
                await ctx.send("❌ Вариант 3 (Opus кодек): НЕ РАБОТАЕТ")
                ctx.voice_client.stop()
        except Exception as e:
            await ctx.send(f"❌ Вариант 3 ошибка: {str(e)}")
        
        await ctx.send("❌ Все варианты не сработали. Возможно проблема с радио потоком или FFmpeg.")
        
    except Exception as e:
        await ctx.reply(f"❌ Ошибка диагностики: {str(e)}")


async def radio_check(ctx):
    """Проверить доступность различных радио потоков"""
    await ctx.send("🔍 Проверяю доступность радио потоков...")
    
    # Список радио потоков для проверки
    radio_streams = {
        "Bluford Radio": "http://bluford.torontocast.com:8085/stream/",
        "Lofi Hip Hop": "http://stream.radiojar.com/4wqre23fytzuv",
        "ChillHop": "http://streams.fluxfm.de/chillhop/mp3-320/audio/",
        "Jazz Radio": "http://jazz-wr04.ice.infomaniak.ch/jazz-wr04-128.mp3",
        "Classical Radio": "http://stream.radiojar.com/0tpy1h0kxtzuv"
    }
            ...
            try:
                async with session.get(url, timeout=10) as response:
                    if response.status == 200:
                        ...
                        results.append(f"✅ **{name}**: Доступно ({response.status}) - {content_type}")
                    else:
                        results.append(f"❌ **{name}**: Недоступно ({response.status})")
            except Exception as e:
                results.append(f"❌ **{name}**: Ошибка - {str(e)[:50]}...")
    
    embed = discord.Embed(
        title="📡 Проверка радио потоков",
        description="\n".join(results),
        color=0x00ff00
    )
...
@bot.command(name='radio_working', aliases=['радио_рабочее'])
async def radio_working(ctx):
    """Включить первое рабочее радио из списка"""
    if ctx.voice_client is None:
        if ctx.author.voice is None:
            await ctx.reply("❌ Подключитесь к голосовому каналу!")
            return
    ...
    # Список радио потоков в порядке приоритета
    radio_streams = [
        {"name": "Lofi Hip Hop", "url": "http://stream.radiojar.com/4wqre23fytzuv"},
        {"name": "ChillHop", "url": "http://streams.fluxfm.de/chillhop/mp3-320/audio/"},
        {"name": "Jazz Radio", "url": "http://jazz-wr04.ice.infomaniak.ch/jazz-wr04-128.mp3"},
        {"name": "Classical Radio", "url": "http://stream.radiojar.com/0tpy1h0kxtzuv"},
        {"name": "Bluford Radio", "url": "http://bluford.torontocast.com:8085/stream/"}
    ]
    
    await ctx.send("🔍 Ищу рабочий радио поток...")
    
    async with aiohttp.ClientSession() as session:
        for radio in radio_streams:
            try:
                # Проверяем доступность
                async with session.get(radio["url"], timeout=10) as response:
                    if response.status == 200:
                        await ctx.send(f"✅ Найден рабочий поток: **{radio['name']}**")
                        ...
                        try:
                            ...
                            def after_playing(error):
                                if error:
                                    print(f'Ошибка радио {radio["name"]}: {error}')
                                else:
                                    print(f'Радио {radio["name"]} остановлено')
                            ...
                            if ctx.voice_client.is_playing():
                                ...
                                embed.add_field(name="Ссылка", value=radio["url"], inline=False)
                                embed.add_field(name="Статус", value="🔊 Воспроизводится", inline=False)
                                embed.set_footer(text=f"Запросил: {ctx.author.display_name}")
                                await ctx.send(embed=embed)
                                return
                            else:
                                await ctx.send(f"⚠️ {radio['name']} подключено, но нет звука")
                                
                        except Exception as e:
                            await ctx.send(f"❌ Ошибка воспроизведения {radio['name']}: {str(e)}")
                            
            except Exception as e:
                await ctx.send(f"❌ {radio['name']}: недоступно")
                continue
    
    await ctx.send("❌ Не удалось найти рабочий радио поток. Попробуйте позже.")
